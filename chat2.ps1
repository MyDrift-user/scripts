...
function EncryptMessage {
    ...
    $key = [System.Text.Encoding]::UTF8.GetBytes("ThisIsASecretKey")  # Use a secure key
    $iv = [System.Text.Encoding]::UTF8.GetBytes("ThisIsAnIV123456")  # Use a secure IV
...
function DecryptMessage {
    ...
    $key = [System.Text.Encoding]::UTF8.GetBytes("ThisIsASecretKey")  # Use the same key
    $iv = [System.Text.Encoding]::UTF8.GetBytes("ThisIsAnIV123456")  # Use the same IV
...
function SendMessage {
    ...
    if ($messageText -ne "" -and $username -ne "") {
        ...
        $encryptedTime = EncryptMessage -plainText $time
        $encryptedUsername = EncryptMessage -plainText $username
        $encryptedText = EncryptMessage -plainText $messageText
        $entry = "$encryptedTime - ${encryptedUsername}: ${encryptedText}`n"
        $logEntry = "OWN::${entry}"
...
function CheckForNewMessages {
    ...
    foreach ($line in $currentContent) {
        if ($line -match "^OWN::") {
            ...
            $parts = $message -split " - "
            $encryptedTime = $parts[0]
            $encryptedUserMessage = $parts[1] -split ": "
            $encryptedUsername = $encryptedUserMessage[0]
            $encryptedText = $encryptedUserMessage[1]
            $decryptedTime = DecryptMessage -cipherText $encryptedTime
            $decryptedUsername = DecryptMessage -cipherText $encryptedUsername
            $decryptedText = DecryptMessage -cipherText $encryptedText
            $messages.Add([ChatMessage]::new($decryptedTime, $decryptedUsername, $decryptedText, "Right"))
        } else {
            $parts = $line -split " - "
            $encryptedTime = $parts[0]
            $encryptedUserMessage = $parts[1] -split ": "
            ...
            $decryptedUsername = DecryptMessage -cipherText $encryptedUsername
            $decryptedText = DecryptMessage -cipherText $encryptedText
            $messages.Add([ChatMessage]::new($decryptedTime, $decryptedUsername, $decryptedText, "Left"))
        }
    }
}
